...
import JoinDiscordCard from '@/components/server/console/JoinDiscordCard'; // Import the new card component
...
const getBackgroundColor = (value: number, max: number | null): string | undefined => {
    ...
    if (delta > 0.8) {
        return delta > 0.9 ? 'bg-red-500' : 'bg-yellow-500';
    }
...
const ServerDetailsBlock = ({ className }: { className?: string }) => {
    ...
    return (
        <>
            ...
            {/* Discord Join Card */}
            <div className={classNames('w-full mt-6', className)}> {/* Ensure full width and margin */}
                <JoinDiscordCard 
                    title="Join Our Community" 
                    description="Astral-Cloud is a small yet suitable hosting provider aiming to achieve simplicity while being fast and affordable."
                    buttonText="Join Now!"
                    link="https://discord.gg/5UDGBjX6fs" // Update with your actual link
                    className="mt-6" // Optional: Additional class for margin or styling
                />
            </div>
        </>
    );
